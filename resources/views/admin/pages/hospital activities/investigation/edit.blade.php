...
<textarea name="description" class="form-control" id="description" rows="7">{{$investigations->description}}</textarea>
</div>
...
@foreach ($doctor as $item)
   <option value="{{$item->id}} {{$item->id == $investigations->doctor_id ? 'selected' : '' }}">{{$item->full_name}}</option>
   @endforeach
  </select>
</div>
<div class="form-group col-6">
