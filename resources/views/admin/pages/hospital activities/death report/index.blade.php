      ...
      @foreach ($deathReport as $key=>$item)
          <tr>
              ...
              <td class="d-print-none">
                ...
                <a class="btn btn-success btn-sm m-1" href="{{route('death_report.show', $item->id)}}"><i class="fas fa-eye"></i></a>
                <a class="btn btn-warning btn-sm m-1" href="{{route('death_report.edit', $item->id)}}"><i class="fas fa-edit"></i></a>
                <form action="{{route('death_report.destroy',$item->id)}}" method="POST">
                    @csrf
                    @method('DELETE')
